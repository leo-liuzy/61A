...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        if scheme_pairp(expr.first):
            operator = scheme_eval(expr.first, env)
            return scheme_apply(operator, rest.map(lambda expr: scheme_eval(expr, env)), env)
        elif scheme_symbolp(expr.first):
            return scheme_apply(env.lookup(first), rest.map(lambda expr: scheme_eval(expr, env)), env)
        elif self_evaluating(expr.first) and expr.second == nil:
            return expr.first
        raise SchemeError(expr)
           # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError(TypeError.__str__)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        param = target.second
        expression = Pair(param, rest)
        env.define(func_name, do_lambda_form(expression, env))
        return func_name
       # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    child = Frame(env)
    head, rest = bindings.first, bindings.second

    for _ in range(len(bindings)):
        check_form(head, 2, 2)
        symbols = Pair(head.first, symbols)
        vals = Pair(scheme_eval(head.second.first, child), vals)
        if rest == nil:
            break
        head, rest = rest.first, rest.second

    check_formals(symbols)
    return child.make_child_frame(symbols, vals)
    # END PROBLEM 15
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    body = expressions.second
    mu_pro = MuProcedure(formals, body)
    return mu_pro
    # END PROBLEM 16
