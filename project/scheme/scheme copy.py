...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    symbols, vals = None, None
    child = Frame(env)
    def make_lsts(the_bindings, the_env):
        if the_bindings == nil:
            return nil
        head, rest = the_bindings.first, the_bindings.second
        check_form(head, 2, 2)
        nonlocal symbols
        nonlocal vals
        symbols = Pair(head.first, make_lsts(rest, the_env))
        val = scheme_eval(head.second.first, the_env)
        vals = Pair(scheme_eval(val, the_env), make_lsts(rest, the_env))
   
    make_lsts(bindings, env)
    check_formals(symbols)
    if bindings.second != nil:
        return scheme_eval(bindings.second, child)
