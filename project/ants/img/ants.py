...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if other.container == False and self.containter and self.ant == None :
            return True
        #elif self.container == False and other.container and other.ant == None:
         #   return True
        else:
            return False
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6B
        if self.digesting == 0:
            #chosen = random_or_none(self.place.bees)
            if self.place.bees:
                self.eat_bee(random_or_none(self.place.bees))
                self.digesting += 1
            #self.place.remove_insect(chosen)
        elif self.digesting == 3:
            self.digesting = 0
        else:
            self.digesting += 1
        # END Problem 6B
