...
def abundant(n):
    ...
    """
    def helper(n, lst, factor):
        if factor == n//2:
            return lst
        elif factor in lst:
            return helper(n, lst, factor+1)
        elif n % factor == 0:
            print(factor,'*', n//factor)
            if n//factor < n:
                return helper(n, lst+[factor, n//factor], factor+1)
            else:
                return helper(n, lst+[factor], factor+1)
        else:
            return helper(n, lst, factor+1)
    the_lst = set(helper(n, [], 1))
    if sum(the_lst)> n:
        return True
    else:
        return False
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def recurse(lnk):
        if lnk == empty:
            ...
            return front+recurse(first(lnk))+mid+recurse(rest(lnk))+back
        else:
            return front+str(first(lnk))+mid+recurse(rest(lnk))+back
