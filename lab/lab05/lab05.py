...
def is_sorted(lst):
    ...
    if not is_link(lst):
        return True
    elif not is_link(first(rest(lst))):
        if first(rest(lst)) == first(lst)+1 or first(rest(lst)) == first(lst):
            return True
        return False

    else:
        if (first(lst) == first(rest(lst)) - 1 or first(lst)==first(rest(lst))) and is_sorted(rest(lst)):
            return True
        return False
