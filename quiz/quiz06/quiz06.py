...
class Player:
    ...
    def debate(self, other):
        gain = max(0.1, self.popularity / (self.popularity + other.popularity)) 
        if random() > gain:
            self.popularity -= 50
...
class Game:
    ...
    def play(self):
        cur_player, other = self.p1, self.p2
        while not self.game_over:
            cur_player.choose(other)(other)
            self.turn += 1
            cur_player, other = other, cur_player
        return self.winner
